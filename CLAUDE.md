...
- `npm run core` — build the WASM core (`core/ && make wasm`) → outputs `public/libminimon.wasm` (always compiled with `-D TRACING`; the in-core classification maps make the tracing build cheap enough to be the only build).
