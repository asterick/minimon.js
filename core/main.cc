...
uint8_t cpu_read_reg(ProcessorState& cpu, uint8_t address) {
	return 0xCD;
}

void cpu_write_reg(ProcessorState& cpu, uint8_t data, uint8_t address) {
}
...
uint8_t cpu_read8(ProcessorState& cpu, uint32_t address) {
	if (address < 0x1000) {
		...
		return cpu_read_reg(cpu, (uint8_t) address);
	} else {
...
void cpu_write8(ProcessorState& cpu, uint8_t data, uint32_t address) {
	if (address >= 0x2100) {
		...
		cpu_write_reg(cpu, data, (uint8_t) address);
